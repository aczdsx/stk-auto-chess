    ...
    public class LobbyBuildingInteractionUI : CachedMonoBehaviour
    {
        ...
        private async UniTask OnClick()
        {
            ...
            if (isInstallFinished)
            {
                ...
                if((gdb.GuideMissionId == 201 && edb.GetFacility((int)IdMap.ElpisBuild.Nest_1)?.IsJustCompleted == true) || edb.GetFacilityLevel(ElpisFacilityType.FacilityTypeNest) >= 1)
                {
                ...
                if(gdb.GuideMissionId == 404 && (edb.GetFacility((int)IdMap.ElpisBuild.Nest_2)?.IsJustCompleted == true) || edb.GetFacilityLevel(ElpisFacilityType.FacilityTypeNest) >= 2) {
                ...
                if(gdb.GuideMissionId == 405 && (edb.GetFacility((int)IdMap.ElpisBuild.DimensionLab)?.IsJustCompleted == true) || edb.GetFacilityLevel(ElpisFacilityType.FacilityTypeDimensionLab) >= 1) {
                    await gdb.AddActionAsync(GuideMissionType.CLEAR_TUTORIAL, 1);
                }
                ...
                if(gdb.GuideMissionId == 407 && (edb.GetFacility((int)IdMap.ElpisBuild.SimulationCenter)?.IsJustCompleted == true) || edb.GetFacilityLevel(ElpisFacilityType.FacilityTypeSimulationCenter) >= 1) {
                    await gdb.AddActionAsync(GuideMissionType.CLEAR_TUTORIAL, 1);
                }
