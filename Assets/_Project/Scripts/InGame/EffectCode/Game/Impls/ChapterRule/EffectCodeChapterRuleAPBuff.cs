            ...
            if (_chapterRuleTiles.Exists(l => l.View.ID == tile.View.ID))
            {
                    ...
                    !_characterByEffectCode.ContainsKey(character.CharacterId))
                {
                    ...
                    var ingameBuffVFX = InGameVfxManager.Instance.AddInGameVfx(
                        InGameEnumExtensions.GetLoopVfxName(BuffDebuffType.None),
                    ...
                    InGameEnumExtensions.GetSoundFx(BuffDebuffType.None);
                    Debug.LogWarning(
                        $"{character.CharacterId} + 버프 추가! | {addBuffInfo.Vfx.name} : 이펙트! | 공격력 : {character.AP}");
                }
                else
                {
        ...
        public override void OnTileCharacterExit(InGameTile tile, CharacterController character)
        {
            ...
            if (_chapterRuleTiles.Exists(l => l == tile))
            {
                    ...
                    _characterByEffectCode.ContainsKey(character.CharacterId))
                {
                    ...
                    Debug.LogWarning(
                        $"{character.CharacterId}  버프 제거!! | {removeBuffInfo.Vfx.name} : 이펙트 제거!! | 공격력 : {character.AP}");
                    removeBuffInfo = null;
                }
                else
                {
