    ...
    public partial class UserDataManager
    {
        ...
        public void UpdateNextRefreshTimeStamp(TermType termType, bool needSave)
        {
            if (termType == TermType.DAILY)
                userQuest.NextDailyRefreshTimestamp = TimeManager.Instance.TommorrowTimeStampLocal();
            else if (termType == TermType.WEEKLY) userQuest.NextWeeklyRefreshTimestamp = TimeManager.Instance.NextMondayTimeStamp();
        ...
        public void SetUserQuestState(int questID, QuestStateType stateType, bool needSave)
        {
            ...
            if (specQuestData.term_type == TermType.DAILY)
            {
                if (userQuest.UserQuestDailyDatas.ContainsKey(questID)) userQuest.UserQuestDailyDatas[questID].QuestStateType = (int)stateType;
            }
            else if (specQuestData.term_type == TermType.WEEKLY)
            {
                if (userQuest.UserQuestWeeklyDatas.ContainsKey(questID)) userQuest.UserQuestWeeklyDatas[questID].QuestStateType = (int)stateType;
            }

            // 완료 요청 처리가 왔을 경우 퀘스트 클리어 마일스톤 업데이트
            if (stateType == QuestStateType.CLEAR) UpdateQuestClearCount(!needSave); // needSave가 true일 경우 함수외부에서 이미 저장 처리를 하기 때문에 false로 처리
        ...
        public int GetQuestClearCount(TermType termType)
        {
            ...
            if (termType == TermType.DAILY)
                foreach (var questData in userQuest.UserQuestDailyDatas)
                {
                    ...
                    if (questData.Value.QuestStateType == (int)QuestStateType.CLEAR) resultCount++;
                }
