...
namespace AutoLetterbox
{
    ...
    public class CameraRatio
    {
        ...
        public void CalculateAndSetCameraRatio(float _width, float _height, bool _horizontalLetterbox)
        {
            ...
            screenViewPortHorizontal.y = Mathf.Lerp(originViewPort.y, originViewPort.y + (originViewPort.height - screenViewPortHorizontal.height),
                vectorAnchor.y);
            ...
            screenViewPortVertical.x = Mathf.Lerp(originViewPort.x, originViewPort.x + (originViewPort.width - screenViewPortVertical.width),
                vectorAnchor.x);
    ...
    public class ForceCameraRatio : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                GameObject findLetterBoxCamera = GameObject.Find("Letter Box Camera");
                if (findLetterBoxCamera == null)
                {
                    ...
                    for (int i = 0; i < cameras.Count; i++)
                    {
                        ...
                        {
                            Debug.Log(cameras[i].camera.name +
        ...
        {
            if (targetCamera != null) // 타겟 카메라가 있을 경우, 1개 카메라만 설정한다
            {
                cameras = new List<CameraRatio>();
                cameras.Add(new CameraRatio(targetCamera, new Vector2(0.5f, 0.5f)));
            }
            else // 타켓 카메라가 없을 경우, 전체 카메라를 찾는다 
            {
