...
namespace Naninovel
{
    ...
    public class CharacterManager : OrthoActorManager<ICharacterActor, CharacterState, CharacterMetadata, CharactersConfiguration>, ICharacterManager
    {
        ...
        public static string UserNickName = "default";
        ...
        public virtual string GetDisplayName (string characterId)
        {
            ...
            // choiJE.Added for localizing
            if (characterId == "Player")
            {
                displayName = UserNickName;
            }
            else if (!string.IsNullOrEmpty(displayName) && displayName.StartsWith("key_"))
            {
                displayName = Localization.GetLocalizedString(displayName.Substring(4));
            }
            else if (!string.IsNullOrEmpty(characterId) && characterId.StartsWith("k_"))
            {
                displayName = Localization.GetLocalizedString($"Char_Name_{characterId.Substring(2)}");
            }
