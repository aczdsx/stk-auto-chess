...
namespace Naninovel
{
    ...
    [InitializeAtRuntime]
    public class ScriptPlayer : IStatefulService<SettingsStateMap>, IStatefulService<GlobalStateMap>,
        IStatefulService<GameStateMap>, IScriptPlayer
    {
        [Serializable]
        ...
        public virtual void SaveServiceState(GameStateMap stateMap)
        {
            var gameState = new GameState
            {
                ...
                GosubReturnSpots =
                    GosubReturnSpots.Count > 0
                        ? GosubReturnSpots.Reverse().ToList()
                        : null // Stack is reversed on enum.
            };
        ...
        public virtual async UniTask LoadServiceStateAsync(GameStateMap stateMap)
        {
            ...
            {
                if (Playlist == null || !PlayedScript ||
                    !stateMap.PlaybackSpot.ScriptName.EqualsFast(PlayedScript.Name))
                {
                ...
                var endIndex = providerConfig.ResourcePolicy == ResourcePolicy.Static
                    ? Playlist.Count - 1
        ...
        public virtual void Play(Script script, int startLineIndex = 0, int startInlineIndex = 0)
        {
            ...
            if (startLineIndex > 0 || startInlineIndex > 0)
            {
                ...
                if (startCommand is null)
                    throw new Error(
                        $"Script player failed to start: no commands found in script `{PlayedScript.Name}` at line #{startLineIndex}.{startInlineIndex}.");
        ...
        public virtual async UniTask PreloadAndPlayAsync(Script script, int startLineIndex = 0,
            int startInlineIndex = 0, string label = null)
        {
            if (!string.IsNullOrEmpty(label))
            {
                if (!script.LabelExists(label))
                    throw new Error(
                        $"Failed navigating script playback to `{label}` label: label not found in `{script.Name}` script.");
        ...
        public virtual async UniTask<bool> RewindAsync(int lineIndex)
        {
            ...
            if (targetCommand is null)
                throw new Error(
                    $"Script player failed to rewind: target line index ({lineIndex}) is not valid for `{PlayedScript.Name}` script.");
        ...
        private async UniTask WaitForInputInAutoPlayAsync()
        {
            ...
            while (AutoPlayActive && WaitingForInput &&
                   Engine.GetService<IAudioManager>()?.GetPlayedVoicePath() != null)
                await AsyncUtils.WaitEndOfFrameAsync();
            if (!AutoPlayActive)
                await WaitForWaitForInputDisabledAsync(); // In case auto play was disabled while waiting for delay.
        }
        ...
        private async UniTask ExecutePlayedCommandAsync(AsyncToken asyncToken)
        {
            ...
            if (providerConfig.ResourcePolicy == ResourcePolicy.Dynamic)
            {
                ...
                if (Playlist.GetCommandByIndex(PlayedIndex + providerConfig.DynamicPolicySteps) is Command.IPreloadable
                    nextPreloadableCmd)
        ...
        private CancellationTokenSource LinkSynchronizationWithContinueInputTokens(
            CancellationToken synchronizationToken)
        {
            ...
            return CancellationTokenSource.CreateLinkedTokenSource(synchronizationToken, continueInputCT, skipInputCT,
                toggleSkipInputCT);
        }
        ...
        private async UniTask ExecuteIgnoringCancellationAsync(Command command, AsyncToken asyncToken)
        {
            try
            {
                await PlayedCommand.ExecuteAsync(asyncToken);
            }
        ...
        private async UniTask<bool> FastForwardRoutineAsync(AsyncToken asyncToken, int targetPlaylistIndex,
            bool executePlayedCommand)
        {
            SetSkipEnabled(true);
        ...
        private bool SelectNextCommand()
        {
            ...
            Engine.Log(
                $"Script '{PlayedScript.Name}' has finished playing, and there wasn't a follow-up goto command. " +
