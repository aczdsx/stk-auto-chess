...
namespace GrpcWebSocketBridge.Client.Unity
{
    ...
    public class UnityWebRequestHttpHandler : HttpMessageHandler
    {
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage requestMessage, CancellationToken cancellationToken)
        {
            ...
            // 강제로 변경
            webRequest.SetRequestHeader("Content-Type", GrpcWebRequestContent.GrpcWebContentType);
			...
			// 200, 201 번은 모두 OK 처리
            var resCode = (HttpStatusCode)webRequest.responseCode;
            if (resCode is >= HttpStatusCode.OK and <= HttpStatusCode.Created)
                resCode = HttpStatusCode.OK;

            var response = new HttpResponseMessage(resCode)
            {
            ...
            HttpResponseHeaders responseTrailingHeaders = response.TrailingHeaders;
            
            foreach (KeyValuePair<string, string> header in webRequest.GetResponseHeaders())
            {
                responseTrailingHeaders.TryAddWithoutValidation(header.Key, header.Value);
            }
            ...
            if (resCode == HttpStatusCode.OK)
            {
                ...
                if (!responseTrailingHeaders.Contains("grpc-message"))
                    responseTrailingHeaders.TryAddWithoutValidation("grpc-message", "OK");
                if (!responseTrailingHeaders.Contains("grpc-status"))
                    responseTrailingHeaders.TryAddWithoutValidation("grpc-status", "0");
            }
